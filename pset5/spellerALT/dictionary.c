...
unsigned int count_num_nodes_recursive(node *head)
{
    unsigned int count = 0;

    count++; // this is the first case in the recursive func; if we get here, we are in a node;

    for (int i = 0; i < CHARSETSIZE; i++)
    {
        if (head -> next_char[i] != NULL)
        {
            count += count_num_nodes_recursive(head -> next_char[i]);
        }
    }
    return count;
}

unsigned int count_num_nodes()
{
    return count_num_nodes_recursive(root);
}
